...
if __name__ == "__main__":
    ...
    # Test 1: Ideal Job (Full Stack React & Node)
    job_good = Job(
        title="Senior Full Stack Developer (React & Node.js)",
        company="Fintech Labs",
        url="https://example.com/jobs/1",
        description="We are building modern web applications with React, Node.js, Express, and PostgreSQL. Experience with WebSockets and Docker is a plus.",
        source="test",
        tags=["React", "Node.js", "PostgreSQL"],
    )

    # Test 2: Irrelevant Job (Customer Support)
    job_bad = Job(
        title="Director of Customer Support",
        company="Cloudbeds",
        url="https://example.com/jobs/2",
        description="Lead a global team of customer support representatives. Experience with Zendesk, Jira, and enterprise client relations required.",
        source="test",
    )

    # Test 3: Disqualified Job (Security clearance requirement)
    job_blocked = Job(
        title="Full Stack Software Engineer",
        company="Defense Tech",
        url="https://example.com/jobs/3",
        description="Develop mission-critical applications using Python and React. Active US Secret Clearance required.",
        source="test",
    )

    for j in [job_good, job_bad, job_blocked]:
        res = matcher.score_job(j)
        print(f"\n--- {j.title} @ {j.company} ---")
